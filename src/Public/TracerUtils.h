...
namespace Stratum::Tracing {
	...
	public:
		LogEntry(LogLevel v_Level, const char* p_Component, std::string&& u_Message,
				 const std::source_location& ro_Location = std::source_location::current())
			: m_Level(v_Level),
			m_Component(p_Component),
			m_Message(std::move(u_Message)),
		...
		inline static LogEntry s_Invalid{ LogLevel::CRASH, "NONE",  std::string("Stratum Tracing") };
	};
	...
	public:
		ExceptionProfile(const char* p_Component, std::string&& u_ErrorMessage,
						 const std::source_location& ro_Location = std::source_location::current())
			: m_Component(p_Component),
			m_ErrorMessage(std::move(u_ErrorMessage)),
			m_Location(ro_Location),
			m_Timestamp(std::chrono::steady_clock::now()),
			m_ThreadID(0) {
		}
		...
		static inline ExceptionProfile s_Invalid{ "Stratum Tracing", std::string("Invalid Exception Profile") };
	};
	...
	public:
		TracingProfile(const char* p_Component, std::string&& u_Label,
					   const std::source_location& ro_Location = std::source_location::current())
			: m_Component(p_Component),
			m_Label(std::move(u_Label)),
			m_Location(ro_Location),
			m_ThreadID(0),
		...
		static inline TracingProfile s_Invalid{ "Stratum Tracing", std::string("Invalid") };
	};
	...
	public:
		LoggerProfile(const char* p_Name, void* p_Buffer, size_t v_BufferSize)
			...
			m_Enabled(true),
			m_TracePolicy(StackTracePolicy::AUTO_FLUSH_ALWAYS) {
		}
